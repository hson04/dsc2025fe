...
const VirtualInterviewer = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: 'bot',
      message: "Hello! I'm your Virtual AI Interviewer. I'll conduct a professional interview with you today based on your CV and Job Description. Are you ready to begin?",
      timestamp: new Date(),
    }
  ])
  ...
  // Handle preparation ready
  const handlePreparationReady = async (extractionStatus) => {
    setPreparationReady(true)
    setShowPreparation(false)
    
    // Update initial message with more context
    const initialMessages = [
      {
        id: 1,
        type: 'bot',
        message: "Great! I've analyzed your CV and Job Description. I'm ready to conduct a comprehensive interview tailored to your background and the position requirements. Let's begin!",
        timestamp: new Date(),
      }
    ]
    setMessages(initialMessages)

    // Silently send language instruction message to agent memory (no UI display)
    setTimeout(async () => {
      ...
      try {
        ...
        // Language instruction sent successfully to agent memory
        // No UI update needed - this is a silent instruction
        console.log('Language instruction sent to agent memory successfully')
      } catch (error) {
        console.error('Language instruction error:', error)
        // Even if there's an error, we continue without showing anything to user
      }
    }, 1500) // Wait 1.5 seconds before sending the silent instruction
  }

  // Handle preparation error
  const handlePreparationError = (error) => {
    console.error('Preparation error:', error)
    // For now, we'll still allow the interview to proceed
    setPreparationReady(false)
    setShowPreparation(false)
  }

  // Show preparation component if not ready
  if (showPreparation) {
    return (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <button
              onClick={() => {
                ...
                // Navigate to preparation page with new session
                window.location.href = '/mock-interview/prepare'
              }}
              style={{
                background: 'rgba(255, 255, 255, 0.1)',
                border: '1px solid rgba(255, 255, 255, 0.2)',
